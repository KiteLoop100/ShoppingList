...
- **prompts/deferred-specials.md** — Same rule summarized for prompts.

### Code changes (not spec files)
- `src/lib/list/special-activation.ts` — `getSpecialActivationCalendarDate`, `computeActivationTime` (12:00 local).
- `src/components/list/hooks/list-data-helpers.ts` — Re-exports; `scheduleActivationTimer` uses new activation instant.
- `src/components/list/hooks/use-auto-reorder.ts`, `use-list-sort.ts` — `available_from` for specials uses activation calendar date for grouping.
- Tests: `src/lib/list/__tests__/special-activation.test.ts`; removed stale `list-data-helpers` mock from `use-auto-reorder.test.ts`.
...
> **Older entries** (2025-02 to 2026-02-26) archived → [archive/CHANGELOG-2025-2026-02.md](archive/CHANGELOG-2025-2026-02.md)
