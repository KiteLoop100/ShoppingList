...
> **Status:** Draft v1  
> **Depends on:** F-RECIPE-SUGGESTIONS (existing spec), Inventory/Pantry system (Nerd-Modus), Product catalog, Shopping list  
...
The existing F-RECIPE-SUGGESTIONS spec covers AI-generated recipes (Modus A: pantry-constrained, Modus B: all recipes). These new features are **complementary**:

- **F-RECIPE-IMPORT** = "I already have a recipe, help me shop for it"
- **F-RECIPE-COOK** = "I don't have a recipe, help me find one based on what I have"
- **F-RECIPE-SUGGESTIONS** = "Generate me a creative recipe from scratch" (existing)

All three share the ingredient→ALDI-product matching engine and pantry comparison logic.
...
interface ImportedRecipe {
  ...
  source_name: string;              // "Chefkoch", "AI-generiert", etc.
  original_servings: number;
  servings_label: string;           // "Portionen", "Stück", etc.
...
CREATE TABLE saved_recipes (
  ...
  source_type TEXT NOT NULL,          -- 'url_import', 'ai_cook', 'ai_suggestion'
  original_servings INTEGER NOT NULL,
  servings_label TEXT DEFAULT 'Portionen',
  ingredients JSONB NOT NULL,         -- RecipeIngredient[]
  instructions JSONB,                 -- RecipeStep[] (may be null for URL imports)
  prep_time_minutes INTEGER,
  cook_time_minutes INTEGER,
  difficulty TEXT,
  aldi_adapted BOOLEAN DEFAULT false, -- was ALDI-Modus used?
...
CREATE TABLE recipe_imports (
  ...
  extraction_method TEXT NOT NULL,    -- 'json-ld', 'microdata', 'ai-fallback'
...
CREATE TABLE cook_conversations (
  ...
  pantry_snapshot JSONB,                  -- pantry state at start
  created_at TIMESTAMPTZ DEFAULT now(),
...
CREATE POLICY "Users see own imports" ON recipe_imports
  FOR ALL USING (auth.uid() = user_id);

ALTER TABLE cook_conversations ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users manage own conversations" ON cook_conversations
  FOR ALL USING (auth.uid() = user_id);
```
...
CREATE INDEX idx_saved_recipes_source ON saved_recipes(user_id, source_type);
CREATE INDEX idx_cook_conversations_user ON cook_conversations(user_id);
```
...
|---|----------|--------|-------|
| Q1 | Web Share API target — does Next.js PWA support this cleanly? | Feature A entry point | Needs spike; may need service worker config |
| Q2 | Rate limit for F-COOK conversations | Cost control | Propose: 10 conversations/day, 10 turns each |
| Q3 | Should F-COOK share the 20/day AI generation limit with F-RECIPE-SUGGESTIONS? | UX + cost | Recommend separate limits since different features |
| Q4 | How to handle recipe instructions for URL imports — scrape or skip? | Feature A completeness | Recommend: extract if available, skip if not (user has the source URL) |
