-- Re-apply the search_retailer_products RPC fix.
-- The function on the remote database still returns category_id UUID
-- from the old schema. Since the return type changed (category_id UUID →
-- demand_group_code TEXT), we must DROP then CREATE — CREATE OR REPLACE
-- cannot change return types.

DROP FUNCTION IF EXISTS search_retailer_products(TEXT, TEXT, UUID, TEXT, INTEGER);

CREATE FUNCTION search_retailer_products(
  p_retailer TEXT,
  p_country TEXT,
  p_user_id UUID,
