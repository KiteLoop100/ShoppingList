...
interface FlyerExtractedData {
  ...
  flyer_country?: string;
  country_uncertain?: boolean;
}
...
export const CaptureStatusFeed = forwardRef<CaptureStatusFeedRef, CaptureStatusFeedProps>(function CaptureStatusFeed(
  ...
  // Polling: while anything is "uploading"/"processing", refetch every 3 s
  ...
  // Once nothing is "in progress" anymore: refetch once so pending_review appears immediately (Realtime may be absent)
  useEffect(() => {
    if (hasInProgress) return;
  ...
  const uploadsNeedingCountry = uploads.filter((u) => {
    const ext = u.extracted_data as FlyerExtractedData | undefined;
    return (
      u.photo_type === "flyer_pdf" &&
      ext?.country_uncertain === true &&
      ext?.flyer_id
    );
  });

  const handleCountrySelect = async (flyerId: string, country: "DE" | "AT", uploadId: string) => {
    try {
      await fetch("/api/flyer-country", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ flyer_id: flyerId, country }),
      });
      setUploads((prev) =>
        prev.map((u) =>
          u.upload_id === uploadId
            ? {
                ...
                extracted_data: {
                  ...
                  flyer_country: country,
                } as PhotoUploadRow["extracted_data"],
              }
            : u
        )
      );
  ...
  return (
    <>
      ...
      {uploadsNeedingCountry.length > 0 && (
        <section className="flex flex-col gap-2">
          {uploadsNeedingCountry.map((u) => {
            const ext = u.extracted_data as FlyerExtractedData;
            return (
              <div
                key={u.upload_id}
              ...
              >
                <p className="mb-3 text-sm font-medium text-amber-800">
                  {t("countryUnknownPrompt")}
                </p>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => handleCountrySelect(ext.flyer_id!, "DE", u.upload_id)}
                    className="flex-1 rounded-xl border-2 border-aldi-blue bg-white px-4 py-3 text-sm font-semibold text-aldi-blue transition-colors hover:bg-aldi-blue hover:text-white"
                  >
                    {t("countryDE")}
                  </button>
                  <button
                    type="button"
                    onClick={() => handleCountrySelect(ext.flyer_id!, "AT", u.upload_id)}
                    className="flex-1 rounded-xl border-2 border-red-500 bg-white px-4 py-3 text-sm font-semibold text-red-600 transition-colors hover:bg-red-500 hover:text-white"
                  >
                    {t("countryAT")}
                  </button>
                </div>
