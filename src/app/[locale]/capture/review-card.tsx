...
export function ReviewCard({ upload, userId, onConfirmed, onDiscarded }: ReviewCardProps) {
  ...
  // Immediately apply fields from extracted_data as soon as data is available (e.g. after polling)
  // For back-side photos: name and brand are NOT taken from the extracted data – only EAN.
  useEffect(() => {
  ...
  // Suggest same product: last confirmed product_front – for product_back (assign back side) and for 2nd/3rd photo of same front
  useEffect(() => {
    const supabase = createClientIfConfigured();
    if (!supabase) return;
    let cancelled = false;
  ...
  // Auto-assign: product_back by EAN; product_front/back when name matches the suggested product (same product across multiple photos)
  useEffect(() => {
    ...
    try {
      const res = await fetch("/api/confirm-photo", {
        ...
        body: JSON.stringify({
          ...
          product: isBackPhoto
            ? {
                // Back photo: only EAN is used; name/brand deliberately omitted
  ...
  return (
    <article className="rounded-xl border border-aldi-muted-light bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-3">
        {isBackPhoto ? (
          <>
            <div className="rounded-lg bg-aldi-muted-light/30 px-3 py-2 text-sm text-aldi-muted">
              Rückseite: Nur EAN-Code wird übernommen. Name und Marke bleiben unverändert.
            </div>
