...
export function ReceiptDonePhase({ t, result, onViewReceipt, onClose }: ReceiptDonePhaseProps) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 p-8">
      <div className={`flex h-16 w-16 items-center justify-center rounded-full ${isMerged ? "bg-aldi-blue" : "bg-green-500"}`}>
        <svg className="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
          {isMerged ? (
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m6.75 12H9.75m3-3H9.75m1.5-9H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
          ) : (
      ...
      <div className="text-center">
        <p className="text-lg font-semibold text-white">
          {isMerged ? t("mergedTitle") : t("success")}
        </p>
        <div className="mt-3 space-y-1">
          ...
          {isMerged ? (
            <p className="text-sm text-white/50">
              {itemsAdded > 0
                ? t("mergedItemsAdded", { count: itemsAdded })
                : t("mergedNoNewItems")}
            </p>
          ) : (
            <p className="text-sm text-white/50">
